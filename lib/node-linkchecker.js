...
exports.linkmap = function(url, opts) {
  ...
  return exports.extract(url, options).then(list => {
    ...
    const processBatch = batch => Promise.all(
      batch.map(u => pagemap(u, options).then(pm => {
        const relativeURL = u.slice(parentURL.length);
        if (pm) {
          return {url: relativeURL, fragments: pm.fragments};
        } else {
          return {url: relativeURL, fragments: []};
        }
      }).catch(() => {
        // we assume the request to sub page failed, we still log the subpage
          const relativeURL = u.slice(parentURL.length);
          return {url: relativeURL, fragments: []};
      })
               )
