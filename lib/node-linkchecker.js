        ...
        if (!cache[urls[index]]) {
          ua.get(urls[index])
            ...
            .end(function(err, res) {
              ...
              cache[urls[index]] = {dom, status: res.status, baseURL: (res.redirects.length > 0) ? res.redirects[res.redirects.length - 1] : urls[index]};
              result.brokenFragments = result.brokenFragments.concat(checkDOMForFragments(dom, urls[index],  fragmentsList[urls[index]], res.status));
              processLink(index + 1);
            });
        } else {
            const {dom, status} = cache[urls[index]];
            result.brokenFragments = result.brokenFragments.concat(checkDOMForFragments(dom, urls[index],  fragmentsList[urls[index]], status));
            processLink(index + 1);
        }
      } else {
        resolve(result);
