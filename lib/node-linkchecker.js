      ...
      .end(function(err, res) {
        if (!res) {
          if (callback) {
            ...
            console.error(chalk.red('Cannot check link: ' + link.href));
          }
        }
        else {
          if (res.headers.location) { // redirect
           ...
           checkLink(link, 'get');
            ...
            if (callback) {
              ...
              console.log(chalk.red('broken link: ' + res.status + ' ' + link.href));
            }
          }
        }
        resolve();
        ...
        ua.get(keys[index])
          ...
          .end(function(err, res) {
            ...
            fragmentsList[keys[index]].forEach(function(hash) {
              ...
              if (!$el.length) {
                if (callback) {
                  callback(keys[index] + hash);
                } else {
                  console.log(chalk.yellow('broken fragment: ' + keys[index] + hash));
                }
              }
            });
            processLink(index + 1);
  ...
  return new Promise(function(resolve, reject) {
    ua.get(url)
      ...
      .end(function(err, res) {
        ...
        Promise.all(p).then(function() {resolve();});
      });
  });
